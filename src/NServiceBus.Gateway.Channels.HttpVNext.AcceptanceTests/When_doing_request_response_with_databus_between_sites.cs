    ...
    public class When_doing_request_response_with_databus_between_sites : NServiceBusAcceptanceTest
    {
        ...
        public async Task Should_be_able_to_reply_to_the_message_using_databus()
        {
            ...
            Assert.AreEqual(PayloadToSend, context.SiteAReceivedPayloadInResponse,
                "The large payload should be marshalled correctly using the databus");
            Assert.AreEqual("httpVNext,http://localhost:25899/SiteA/", context.OriginatingSiteForRequest);
            Assert.AreEqual("httpVNext,http://localhost:25899/SiteB/", context.OriginatingSiteForResponse);
        ...
        public class SiteA : EndpointConfigurationBuilder
        {
            public SiteA()
            {
                EndpointSetup<DefaultServer>(c =>
                {
                    ...
                    gatewaySettings.AddReceiveChannel("http://localhost:25899/SiteA/", "httpVNext");
                    gatewaySettings.AddSite("SiteB", "http://localhost:25899/SiteB/", "httpVNext");
                });
            }
        ...
        public class SiteB : EndpointConfigurationBuilder
        {
            public SiteB()
            {
                EndpointSetup<DefaultServer>(c =>
                {
                    ...
                    gatewaySettings.AddReceiveChannel("http://localhost:25899/SiteB/", "httpVNext");
                });
            }
